...
The configuration command mounts the Livox Mid-360 on `livox_frame`, removes all UnitySensors `TFLink` components, and publishes its `sensor_msgs/msg/PointCloud2` data on `/livox/lidar`. It preserves the UnitySensors Mid-360 defaults of 20,000 points at 10 Hz with a 0.1-70 m range. Unity also publishes `/clock`, `/joint_states`, only the dynamic `odom -> base_footprint` TF edge, and subscribes to `/cmd_vel` through the project-owned skid-steer actuator. Arm command subscribers remain deferred; without a separate arm controller, the arm joints are passive in Play Mode.
